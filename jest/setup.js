...
module.exports = async globalConfig => {
  ...
  try {
    ...
    if (error.stderr && !error.stdout.includes("ERROR")) {
      ...
      // Dump the errors in stdout generated by 'errorf' into a file so we can use it to assert in tests
      if (error.stdout.includes("ERROR")) {
        console.log("Writing to", path.join(jestHugoConfig.publishDir, 'output.err'));
        fs.writeFileSync(path.join(jestHugoConfig.publishDir, 'output.err'), error.stdout);
      }
